document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateForm() {
        const requiredFields = ['nomeEvento', 'dataEvento', 'localEvento', 'nomeSolicitante', 'unidade'];
    ...
    function adicionarParticipante(nome, tipo) {
        ...
        const participanteExistente = savedData.participantes.find(p => p.nome === nome);
    ...
    function removerParticipante(nome) {
        ...
        savedData.participantes = savedData.participantes.filter(p => p.nome !== nome);
        localStorage.setItem('formData', JSON.stringify(savedData));
        
        const payments = JSON.parse(localStorage.getItem('payments')) || {};
        delete payments[nome];
    ...
    document.getElementById('adicionarParticipante').addEventListener('click', () => {
        ...
        function updateAbrirFormButtonState() {
            abrirFormButton.disabled = !(nomeInput.value.trim() && tipoSelect.value === 'Expert Externo');
        }

        nomeInput.addEventListener('input', updateAbrirFormButtonState);
        tipoSelect.addEventListener('change', updateAbrirFormButtonState);
        ...
        tipoSelect.addEventListener('change', () => {
            updateAbrirFormButtonState();
            
            const payments = JSON.parse(localStorage.getItem('payments')) || {};
            if (payments[nomeInput.value.trim()]) {
    ...
    function restoreFormData() {
        ...
        if (storedActivities) {
            ...
            activities.forEach(activity => {
                adicionarAtividade(activity);
            });
        }
    }


function adicionarAtividade(activityData = {}) {
...
function limparDadosFormulario() {
    ...
    savedData = { participantes: [] };
    todosOsParticipantes.clear();
    atualizarParticipantesDisponiveis();
}

document.getElementById('salvarFormulario').addEventListener('click', () => {
    const eventDetails = {
        nomeEvento: document.getElementById('nomeEvento').value,
        dataEvento: document.getElementById('dataEvento').value,
        localEvento: document.getElementById('localEvento').value,
